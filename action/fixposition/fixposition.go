...
// File :
func File(f *ast.File, base token.Pos) {
...
// Decl :
func Decl(decl ast.Decl, base token.Pos) {
	var offset int
	switch t := decl.(type) {
	case *ast.FuncDecl:
		if t.Doc != nil {
			offset = int(-t.Doc.Pos() + base)
		} else {
			offset = int(-t.Pos() + base)
		}
	case *ast.GenDecl:
		if t.Doc != nil {
			offset = int(-t.Doc.Pos() + base)
		...
		if node != nil {
			...
			case *ast.FieldList:
				if x.Opening.IsValid() {
					...
					x.Closing = token.Pos(int(x.Closing) + offset)
				}
			...
			case *ast.GenDecl:
				...
				if x.Rparen != token.NoPos {
					x.Rparen = token.Pos(int(x.Rparen) + offset)
				}
			case *ast.FuncDecl:
				// d.Type.Pos()
			case *ast.File:
